...
fetch(`https://jsonplaceholder.typicode.com/users`)
    ...
    .then(users => {
        ...
        for (const user of users) {
            ...
            userDetailBtn.className = `button-53`
            userDiv.className = `userDiv`
            userIdDiv.innerHTML = `<h2>user id: ${user.id}</h2>`;
            userNameDiv.innerHTML = `<h3> user name: ${user.name} </h3>`;
            userDetailBtn.innerHTML = `DETAIL`;


            userDiv.append(userIdDiv, userNameDiv, userDetailBtn)
